...
if (!isset($submit)) {
	...
	//Preventing sql injection when adding a user
	$conn = new mysqli($mysqlServer, $mysqlUser, $mysqlPassword, $mysqlMainDb);
	mysqli_set_charset($conn,"utf8");
	
	$statement=$conn->prepare("INSERT INTO `$mysqlMainDb`.user
								(user_id, nom, prenom, username, password, email, statut, department, am, registered_at, expires_at, lang)
								VALUES ('NULL', ?, ?, ?, '$password_encrypted', ?,'5',
								?, ?,".$registered_at.",".$expires_at.",'$lang')");
	
	$statement->bind_param("ssssis", $nom_form, $prenom_form, $uname, $email, $department, $am);
	$statement->execute();
	
	$inscr_user=$statement->get_result();
