...
## Check if there is html in input field
...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
    ...
    //check if html code in inputed
    if(isHTML($prenom_form) OR isHTML($nom_form) OR isHTML($email_form) OR isHTML($am_form)) {
...
if ((!isset($changePass)) || isset($_POST['submit'])) {
	...
	$tool_content .= "</ul></div>";
	$tool_content .= "<form method=\"post\" action=\"$sec?submit=yes\"><br/>".csrf_token()."
    <table width=\"99%\">
    <tbody><tr>
